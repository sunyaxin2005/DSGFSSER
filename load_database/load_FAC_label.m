function [ files_path files_people_label files_emotion_label files_sentence_label words_mark ] = load_FAC_label( wave_files_names, wave_files_path )
%LOAD_FAC_LABEL Summary of this function goes here
...
load FAU_files_name
%save  FAU_files_name files_name
switch load_type 
    case 'load_all'
...
for ci = 1:1:load_file_num
    ...
    ind = find(file_name == '_');
    files_people_label{index} = file_name(1:ind(2) - 1);
    
    files_emotion_label{index} =gnd(ci);
    files_sentence_label{index} = file_name(1:ind(1) - 1);
