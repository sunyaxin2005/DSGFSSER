function [ stat_res ] = spr_stat_one_dim(features, speech_label)
...
for feature_index = 1 : features_dim
    ...
    if length(undiff_stats_featue) < 3
        stat_res(feature_index, :) = [NaN, NaN, NaN, NaN];
        continue;
    end
    [mean_res std_res] = max_min_mean_std_feature( undiff_stats_featue);
    stat_res(feature_index, 1 : 2) = [mean_res, std_res];
    ...
    [mean_res std_res] = max_min_mean_std_feature(diff_stats_featue);
    stat_res(feature_index, 3 : 4) = [mean_res, std_res];
...
%     [mean_res std_res] = max_min_mean_std_feature(diff_diff_stats_featue);
%     stat_res(feature_index, 5 : 6) = [mean_res, std_res];   
end
end

function [mean_res std_res] = max_min_mean_std_feature( features)
if(isempty(features))
    mean_res = 0;
    std_res = 0;
else
