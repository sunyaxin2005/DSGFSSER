function [ cLbest,maxJ ] = mc_sequentialforwardfloatingselection( features, label, CostFunction, NumFeatComb )
%MC_SEQUENTIALFORWARDFLOATINGSELECTION Summary of this function goes here
...
k=2;
% Initialization
[X{k},C{k}]=mc_SequentialForwardSelection(features,label,CostFunction,2);
...
while k<=NumFeatComb
    ...
    for i=1:length(Y{m-k})
        t=[X{k} Y{m-k}(i)];
        sel_f = features(t, :);
        Ct=[Ct eval([CostFunction '(sel_f, label);'])];
    ...
    xr=X{k+1}(r);
    if r==k+1
        sel_f = features(X{k+1}, :);
        C{k+1}=eval([CostFunction '(sel_f, label);']);
    ...
    while flag
        ...
        if J<C{k-1}
            X{k}=X_hat{k};
            sel_f = features(X{k}, :);
            C{k}=eval([CostFunction '(sel_f, label);']);
