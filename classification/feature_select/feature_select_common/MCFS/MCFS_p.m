...
% Y = GCMDistance(Y', fea');
% fea = GCMDistance(fea', fea')';
%[weight,x] = L2MCC(fea',Y,0.001);
...
function [weight,x] = L2MCC(A,y,gamma1)
    
    weight = ones(size(A,1),1);
    
    for i=1:10
        
      A1 = repmat(sqrt(weight),1,size(A,2)).*A;    
      y1 = sqrt(weight).*y;
        
      x = (A1'*A1+gamma1*diag(ones(size(A,2),1)))\(A1'*y1);
