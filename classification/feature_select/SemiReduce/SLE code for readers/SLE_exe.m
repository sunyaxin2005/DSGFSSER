...
A=imreadYale10080(15,11);B=A(1:4:100,1:4:80,:);classnum=15;totlenuminclass=11;trainingnuminclass=6; Kneighbor=6;form=0;delta=1;energyordim=0.95;load randvector50by11;X=reshapeimageCOLtovector(B);%X:each column is a pattern vector
k=0;PCAdim=[0 30 40 40 40 40];rateYale_SPCA4to6d1rand10=[];

for trainingnuminclass=3:6
     for traintime=1:10
            randvector=zeros(1,totlenuminclass);randvector=randvector50by11(traintime,:);%randvector_UCIwine10by48(traintime,:);
            count=0;ind=0;clear Xrand gnd i j;
            for i=1:classnum
...
%                 %projection=zeros(ilineeigenvector,dim);    
                ...
                Ytrain=projection'*Xtrain;
                Ytest=projection'*Xtest;%
...
%plot(Yplot(1,1:48),Yplot(2,1:48),'.',Yplot(1,49:96),Yplot(2,49:96),'+',Ypl
%ot(1,96:end),Yplot(2,96:end),'o')