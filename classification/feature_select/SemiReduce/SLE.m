...
RV =  Ll/trace(Ll) + b * E /trace(E ) + b.*LG;
lambda = 1000;
maxiter = 50;
trace1 = 1;
stop = 2;
XX = XL * LW * XL'  ;            
Nprojection=150;
[sl sv pcal pcav plots] = spca([], RV, Nprojection, lambda, -stop*15, maxiter, trace1);
              
sl=fliplr(sl);  % for the stability and obtaining the sparse projection of SLE
W=[sl ones(size(sl,1),1)];%just a trick in dimensionality reduction to achieve the expected number of projections (for compensation)
